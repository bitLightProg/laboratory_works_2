...
/*Выводит на экран состояние системы.*/
int print_memory(block_list* first_free_block, block_list* first_occupied_block)
{
	...
	cout << sys << " системных блоков свободной памяти." << endl;
	int a = sys;
	...
	cout << sys - a << " системных блоков занятой памяти." << endl;
	...
	/*Используются для предварительного форматирования.*/
...
/*Переносит блоки поближе к началу списка.*/
...
void* my_malloc(int size)
{
	...
	block_list* current_free_block = first_free_block;
	block_list* prev_free_block = first_free_block;
	/*Ищем крайний элемент списка свободной памяти.*/
	while (current_free_block->next_element != NULL)
	{
		prev_free_block = current_free_block;
		current_free_block = current_free_block->next_element;
	}
	/*Если крайний элемент свободной памяти пустой,
	а крайний элемент занятой памяти не пустой(т.е. последний) - переносим блок.*/
	if (current_free_block->block == NULL && current_occupied_block->block != NULL)
	{
		prev_free_block->next_element = NULL;
		current_occupied_block->next_element = current_free_block;
		current_occupied_block = current_occupied_block->next_element;
	}
	else
	/*Иначе создаём новый.*/
...
/*Вывод разметки кучи для отладки.*/
...
{	
	...
	while (current_free_block != NULL && current_free_block->block != NULL)
	{
		...
		/*И здесь, наверняка, можно было использовать goto для более понятного перехода,
		но я сначала посчитал, что это будет плохо.*/
		if (flag == 1)
		{
			current_free_block = current_free_block->next_element;
			continue;
	...
	after_reverse: after_sys_merge:
		...
		if (current_free_block->block < second_free_block->block)
		{
				...
				second_free_block->block)
			{
				...
				free_system_blocks();
				current_free_block = first_free_block;
				/*Цикл проверки возможности слияния начинается заново.*/
				continue;
			}
		...
		else if (current_free_block->block > second_free_block->block)
		{
			...
			current_free_block = second_free_block;
			second_free_block = (block_list*)pointer;
			goto after_reverse; // Это удобно и понятно.
		}
...
/*Функция, сбрасывающая разметку кучи.*/
int free_all()
{
	system("cls");
	p_count = 0;
...
/*Функция запросов на освобождение и добавление блоков.*/
int remap_blocks()
{
	system("cls");
	if (p_count == 0)
